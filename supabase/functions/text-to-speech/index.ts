...
async function uploadAudioToStorage(stream, requestHash) {
  const { data, error } = await supabase.storage.from('sophia-tts-audio') // CHANGE: Your bucket name
...
Deno.serve(async (req)=>{
  ...
  let text, voiceId;
  // Handle POST request from your app
  if (req.method === 'POST') {
  ...
  const { data } = await supabase.storage.from('sophia-tts-audio').createSignedUrl(`${requestHash}.mp3`, 60); // CHANGE: bucket name
  if (data) {
